...
namespace dev.kokoa.UploadNotification
{
    ...
    public class UploadNotificationSettings
    {
        ...
        public string GetSuccessSoundPath()
        {
            ...
            // Return the resource name (without extension) so PlayNotificationSound
            // can attempt Resources.Load<AudioClip>(name) when appropriate.
            return $"Success-{(int)successSelection + 1}";
        }
        ...
        public string GetErrorSoundPath()
        {
            ...
            // Return the resource name (without extension) so PlayNotificationSound
            // can attempt Resources.Load<AudioClip>(name) when appropriate.
            return $"Fail-{(int)errorSelection + 1}";
        }
...
";
        ...
        private static void PlayNotificationSound(bool isSuccess = true)
        {
            ...
            if (!string.IsNullOrEmpty(soundPath))
            {
                ...
                // Resources フォルダ内の AudioClip を試す
                else
                {
                    var resClip = Resources.Load<AudioClip>(soundPath);
                    if (resClip != null)
                    {
                        PlayClip(resClip, volume);
                        return;
                    }
                }
            }
